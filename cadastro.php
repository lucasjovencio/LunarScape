...
<?php
	...
	{
	 	require(dirname(__FILE__)."/src/class/bd.class.php");
		$bd = new Db();
		...
		$results = $bd->single("SELECT username FROM usuarios WHERE username=? ",array($username));
		if ($username == $results)
		{
		...
		$check = $bd->query("INSERT INTO usuarios(usuario_id, fullname,email,username,senha,nivel_usuario,data_cadastro,data_ultimo_login,active,key_activate) VALUES 			(NULL,?,?,?,?,'0','2016-01-01 00:00:00.000000','2016-01-01 00:00:00.000000','0',?)", array($newfullname,$newemail,$newusername,$crypt_pass,$key_activate));
		if($check == NULL){
